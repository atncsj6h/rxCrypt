/*  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    sha2.h,sha2.c
    Implementation of the SHA256,SHA384,SHA512 hash(es)
...
#define ROR(value, bits, arch)  (((value) >> (bits)) | ((value) << ((arch)- (bits))))
...
#define S(x, n)         ROR((x),(n),32)
#define R( x, n )       (((x)&0xFFFFFFFFUL)>>(n))
#define S0gma( x )      (S(x, 2) ^ S(x, 13) ^ S(x, 22))
#define S1gma( x )      (S(x, 6) ^ S(x, 11) ^ S(x, 25))
#define G0mma( x )      (S(x, 7) ^ S(x, 18) ^ R(x, 3))
#define G1mma( x )      (S(x, 17) ^ S(x, 19) ^ R(x, 10))

#define sha256Round( a, b, c, d, e, f, g, h, i ) \
     t0 = h + S1gma(e) + Ch(e, f, g) + K256[i] + W[i]; \
     t1 = S0gma(a) + Maj(a, b, c); \
     d += t0; \
     h  = t0 + t1;

/*  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    sha512  constants and macros
*/
// The K array
...
#define S64( x, n )         ROR((x),(n),64 )
#define R64( x, n )         (((x)&0xFFFFFFFFFFFFFFFFULL)>>((uint64_t)n))
#define S0gma64( x )        (S64(x, 28) ^ S64(x, 34) ^ S64(x, 39))
#define S1gma64( x )        (S64(x, 14) ^ S64(x, 18) ^ S64(x, 41))
#define G0mma64( x )        (S64(x, 1) ^ S64(x, 8) ^ R64(x, 7))
#define G1mma64( x )        (S64(x, 19) ^ S64(x, 61) ^ R64(x, 6))

#define sha512Round( a, b, c, d, e, f, g, h, i ) \
     t0 = h + S1gma64(e) + Ch(e, f, g) + K512[i] + W[i]; \
     t1 = S0gma64(a) + Maj(a, b, c); \
     d += t0; \
     h  = t0 + t1;
...
void    sha256Transform(sha256Context* Context, uint8_t const* Buffer )
{
    ...
    for( i=16; i<64; i++ )
    {
        W[i] = G1mma( W[i-2]) + W[i-7] + G0mma( W[i-15] ) + W[i-16];
    }
...
void    sha256Update( sha256Context* Context, void const* Buffer, uint32_t BufferSize )
{
    ...
    while( BufferSize > 0 )
    {
        ...
        {
           sha256Transform( Context, (uint8_t*)Buffer );
        ...
        {
           n = MIN( BufferSize, (SHA256_BLOCK_SIZE  - Context->curlen) );
...
void sha256Last( sha256Context * Context, SHA256_HASH * sha256Hash )
{
    int i ;
    // Copy output
    for( i=0; i<8; i++ )
    {
        STORE32H( Context->state[i], sha256Hash->bytes+(4*i) );
    }
}
...
{
    sha256Finalise( Context );
}

void sha224Last( sha256Context * Context, SHA224_HASH * sha224Hash )
{
    int i ;
    // Copy output
    for( i=0; i<7; i++ )
...
void    sha512Transform( sha512Context* Context, uint8_t const * Buffer )
{
    ...
    for( i=16; i<80; i++ )
    {
        W[i] = G1mma64(W[i-2]) + W[i-7] + G0mma64(W[i-15]) + W[i-16];
    }
...
void    sha512Update( sha512Context* Context, void const* Buffer, uint32_t BufferSize )
{
    ...
    while( BufferSize > 0 )
    {
        ...
        {
           sha512Transform( Context, (uint8_t *)Buffer );
...
void sha512Last( sha512Context * Context, SHA512_HASH * sha512Hash )
{
    int i ;
    // Copy output
    for( i=0; i<8; i++ )
    {
        STORE64H( Context->state[i], sha512Hash->bytes+(8*i) );
    }
}
...
{
    sha512Finalise( Context );
}

void sha384Last( sha512Context * Context, SHA384_HASH * sha384Hash )
{
    int i ;
    // Copy output
    for( i=0; i<6; i++ )
