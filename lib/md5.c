...
/*  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    for architectures that lack an AND-NOT instruction
*/
...
/*  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    md5Transform
    processes one or more 64-byte data blocks,
    does NOT update the bit counters
    no alignment requirements.
*/
static
void * md5Transform( md5Context * ctx, void const * data, uintmax_t size )
{
    uint8_t *   ptr;
    uint32_t    a;
    uint32_t    b;
...
void    md5Update( md5Context * Context, void const * Buffer, uint32_t BufferSize )
{
    ...
    if( BufferSize >= 64 )
    {
        Buffer = md5Transform( Context, Buffer, BufferSize & ~(unsigned long)0x3f );
        BufferSize &= 0x3f;
    }
...
/*  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    After calling this, md5Initialise must be used to reuse the context.
*/
